...
Test(options_parser, get_server_options_valid)
{
    ...
    cr_assert_not_null(options,
        "get_server_options should not return NULL for valid arguments");
    cr_assert_eq(
        options->error, false, "Error should be false for valid arguments");
    ...
    cr_assert_eq(
        options->clients_nb, 5, "Clients number should be set correctly");
    cr_assert_eq(options->frequency, 100, "Frequency should be set correctly");
    cr_assert_not_null(options->teams, "Teams should be set");
    cr_assert_str_eq(
        options->teams[0], "team1", "First team should be correct");
    cr_assert_str_eq(
        options->teams[1], "team2", "Second team should be correct");
...
{
    char *argv[] = {"zappy_server", "-p", "8080", "-x", "10", "-y", "15", "-n",
        "team1", "team2", "-c", "5", NULL};
    int argc = 9;
...
Test(options_parser, get_server_options_missing_mandatory)
{
    ...
    cr_assert_not_null(
        options, "get_server_options should return pointer even on error");
    cr_assert_eq(options->error, true,
        "Error flag should be true for missing mandatory options");
}
...
Test(options_parser, get_server_options_help_request)
{
    ...
    cr_assert_not_null(
        options, "get_server_options should not return NULL for help request");
...
Test(options_parser, get_server_options_debug_request)
{
    ...
    cr_assert_not_null(options,
        "get_server_options should not return NULL for debug request");
...
{
    char *argv[] = {"zappy_server", "-p", "0",      // Invalid port
        "-x", "10", "-y", "15", "-n", "team1"};
    int argc = 9;
    ...
    cr_assert_not_null(
        options, "get_server_options should return pointer even on error");
    cr_assert_eq(options->error, true,
        "Error flag should be true for invalid arguments");
...
{
    char *argv[] = {"zappy_server", "--debug", "-p", "9999", "-x", "25", "-y",
        "30", "-n", "alpha", "beta", "gamma", "delta", "-c", "10", "-f",
        "500"};
    int argc = 17;
    ...
    cr_assert_not_null(
        options, "get_server_options should handle complex scenario");
    cr_assert_eq(options->error, false, "Error flag should be false");
    cr_assert_eq(options->debug, true, "Debug flag should be set");
    ...
    cr_assert_eq(
        options->clients_nb, 10, "Clients number should be set correctly");
    ...
    cr_assert_str_eq(
        options->teams[3], "delta", "Fourth team should be delta");

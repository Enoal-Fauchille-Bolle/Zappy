...
Test(pbc_event, valid_player_and_message, .timeout = 2) {
    ...
    cr_assert(strstr(response, "pbc #7 hello world") != NULL, "Should output 'pbc #7 hello world' for valid player and message");
    close(read_fd);
    free(response);
    free(player);
...
Test(pbc_event, null_player, .timeout = 2) {
    pbc_event(NULL, "msg");
}

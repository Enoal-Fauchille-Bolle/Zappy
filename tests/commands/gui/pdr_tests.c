...
// --- Mock helpers (reuse from other GUI event tests) ---
...
Test(pdr_event, valid_player_and_resource, .timeout = 2) {
    ...
    cr_assert(strstr(response, "pdr #5 1") != NULL, "Should output 'pdr #5 1' for player 5 and resource LINEMATE");
    close(read_fd);
    free(response);
    free(player);
    free(team);
...
Test(pdr_event, null_player, .timeout = 2) {
    pdr_event(NULL, FOOD);
}

Test(pdr_event, invalid_resource, .timeout = 2) {
    server_t *server = create_test_server();
    game_t *game = create_test_game(server);
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    ...
    pdr_event(player, RESOURCE_COUNT); // Invalid resource
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert(response == NULL || strstr(response, "pdr #8") == NULL, "Should not output for invalid resource");
    close(read_fd);
    free(player->team);
    free(player);
    destroy_test_client(client);

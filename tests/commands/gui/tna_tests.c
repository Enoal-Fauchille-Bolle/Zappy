...
Test(tna_command, outputs_all_teams, .timeout = 2) {
    server_t *server = create_test_server();
    server->options->teams = malloc(3 * sizeof(char *));
    server->options->teams[0] = strdup("TeamA");
    server->options->teams[1] = strdup("TeamB");
    server->options->teams[2] = NULL;
    ...
    command_t cmd = { .name = "tna", .argc = 0, .argv = NULL };
    tna_command(client, &cmd);
    ...
    cr_assert(strstr(response, "tna TeamA") != NULL, "Should output tna for TeamA");
    cr_assert(strstr(response, "tna TeamB") != NULL, "Should output tna for TeamB");
    close(read_fd);
    free(response);
    free(server->options->teams[0]);
    free(server->options->teams[1]);

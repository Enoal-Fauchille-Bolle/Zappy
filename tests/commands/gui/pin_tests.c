...
Test(pin_event, valid_player, .timeout = 2) {
    ...
    player->pos.x = 5;
    player->pos.y = 6;
    for (int i = 0; i < RESOURCE_COUNT; i++) player->inventory[i] = i;
    pin_event(player);
    ...
    cr_assert(strstr(response, "pin #123 5 6") != NULL, "Should output correct pin event");
    close(read_fd);
    free(response);
    free(player);
...
Test(pin_event, null_player, .timeout = 2) {
    pin_event(NULL);
}

// --- pin_command tests ---

Test(pin_command, valid_player, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
    ...
    server->game = game;
    client->server = server;
    command_t cmd = { .argc = 1, .argv = (char *[1]){"123"} };
    pin_command(client, &cmd);
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert_not_null(response, "Response should not be NULL");
    cr_assert(strstr(response, "pin #123 5 6") != NULL, "Should output correct pin command");
    close(read_fd);
...
Test(pin_command, invalid_argc, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
...
Test(pin_command, invalid_player_id, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
...
Test(pin_command, player_not_found, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);

...
Test(pgt_event, valid_player_and_resource, .timeout = 2) {
    ...
    cr_assert(strstr(response, "pgt #7 0") != NULL, "Should output 'pgt #7 0' for FOOD resource");
    close(read_fd);
    free(response);
    free(player->team);
    free(player);
...
Test(pgt_event, null_player, .timeout = 2) {
    pgt_event(NULL, FOOD);
}

Test(pgt_event, invalid_resource, .timeout = 2) {
    server_t *server = create_test_server();
    game_t *game = create_test_game(server);
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    ...
    cr_assert_not_null(player->team, "Team allocation failed");
    player->team->game = game;
    pgt_event(player, RESOURCE_COUNT); // Invalid resource
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert(response == NULL || strstr(response, "pgt #8") == NULL, "Should not output for invalid resource");
    close(read_fd);
    free(player->team);
    free(player);
    destroy_test_client(client);
...
Test(pgt_event, null_team_or_game_or_server, .timeout = 2) {
    player_t player = {0};
    // Null team
    pgt_event(&player, FOOD);
    // Null game

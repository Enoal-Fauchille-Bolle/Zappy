...
Test(ppo_event, valid_player, .timeout = 2) {
    ...
    cr_assert(strstr(response, "ppo #88 2 3 2") != NULL, "Should output correct ppo event");
    close(read_fd);
    free(response);
    free(player);
...
Test(ppo_event, null_player, .timeout = 2) {
    ppo_event(NULL);
}

Test(ppo_command, valid_player, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
    ...
    command_t cmd = { .argc = 1, .argv = (char *[1]){"88"} };
    ppo_command(client, &cmd);
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert_not_null(response, "Response should not be NULL");
    cr_assert(strstr(response, "ppo #88 2 3 2") != NULL, "Should output correct ppo command");
    close(read_fd);
...
Test(ppo_command, invalid_argc, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
...
Test(ppo_command, invalid_player_id, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
...
Test(ppo_command, player_not_found, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);

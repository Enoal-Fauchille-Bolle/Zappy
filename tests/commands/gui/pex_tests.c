...
Test(pex_event, valid_player, .timeout = 2)
{
    ...
    cr_assert(strstr(response, "pex #17") != NULL,
        "Should output 'pex #17' for valid player");
    close(read_fd);
    free(response);
    free(player);
...
Test(pex_event, null_player, .timeout = 2)
{
    pex_event(NULL);
}

Test(pex_event, null_client, .timeout = 2)
{
    player_t player = {0};
    player.client = NULL;
    pex_event(&player);

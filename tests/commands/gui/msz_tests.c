...
// --- Mock helpers (reuse from other GUI command tests) ---
...
static server_t *create_test_server(void)
{
    ...
    server->options->width = 12;
    server->options->height = 34;
...
Test(msz_command, valid_map_size, .timeout = 2) {
    ...
    command_t cmd = { .name = "msz", .argc = 0, .argv = NULL };
    msz_command(client, &cmd);
    ...
    cr_assert(strstr(response, "msz 12 34") != NULL, "Should output 'msz 12 34' for map size");
    close(read_fd);
    free(response);

...
Test(sgt_command, valid_tick_rate, .timeout = 2) {
    ...
    command_t cmd = { .name = "sgt", .argc = 0, .argv = NULL };
    sgt_command(client, &cmd);
    ...
    cr_assert(strstr(response, "sgt 42") != NULL, "Should output correct sgt tick rate");
    close(read_fd);
    free(response);

...
Test(pie_event, success_true, .timeout = 2) {
    ...
    cr_assert(strstr(response, "pie 2 3 1") != NULL, "Should output 'pie 2 3 1' for success true");
    close(read_fd);
    free(response);
...
Test(pie_event, success_false, .timeout = 2) {
    server_t *server = create_test_server();
    client_t *client = create_test_client(server);
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
    ...
    server->clients[0] = client;
    pos_t pos = {5, 6};
    pie_event(pos, false, server);
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert_not_null(response, "Response should not be NULL");
    cr_assert(strstr(response, "pie 5 6 0") != NULL, "Should output 'pie 5 6 0' for success false");
    close(read_fd);
    free(response);
    destroy_test_client(client);
    destroy_test_server(server);
...
Test(pie_event, null_server, .timeout = 2) {
    pos_t pos = {1, 1};
    pie_event(pos, true, NULL);
}

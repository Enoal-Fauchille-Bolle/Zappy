...
Test(sst_command, valid_tick_speed, .timeout = 2) {
    ...
    command_t cmd = { .name = "sst", .argc = 1 };
    char *argv[] = { "200" };
    cmd.argv = argv;
    sst_command(client, &cmd);
    ...
    cr_assert(strstr(response, "sst 200") != NULL, "Should output correct sst tick speed");
    close(read_fd);
    free(response);
...
Test(sst_command, missing_argument, .timeout = 2) {
    server_t *server = create_test_server();
    game_t *game = malloc(sizeof(game_t));
    cr_assert_not_null(game);
    game->tick_rate = 100;
    ...
    client->sockfd = write_fd;
    server->clients[0] = client;
    command_t cmd = { .name = "sst", .argc = 0, .argv = NULL };
    sst_command(client, &cmd);
    flush_client_writing_buffer(client);
    close(write_fd);
    char *response = read_from_pipe(read_fd);
    cr_assert_not_null(response, "Response should not be NULL");
    cr_assert(strstr(response, "sbp") != NULL, "Should output sbp for missing argument");
    close(read_fd);
    free(response);
    destroy_test_client(client);
    free(game);
...
Test(sst_command, zero_tick_speed, .timeout = 2) {
    server_t *server = create_test_server();
    game_t *game = malloc(sizeof(game_t));
    cr_assert_not_null(game);
    game->tick_rate = 100;
    ...
    char *response = read_from_pipe(read_fd);
    cr_assert_not_null(response, "Response should not be NULL");
    cr_assert(strstr(response, "sbp") != NULL, "Should output sbp for zero tick speed");
    close(read_fd);
    free(response);
    destroy_test_client(client);
    free(game);

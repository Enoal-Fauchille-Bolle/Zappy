...
Test(pnw_event, valid_player, .timeout = 2) {
    ...
    cr_assert(strstr(response, "pnw #55 4 5 3 3 team1") != NULL, "Should output correct pnw event");
    close(read_fd);
    free(team->name);
    free(team);
    free(response);
...
Test(pnw_event, null_player, .timeout = 2) {
    pnw_event(NULL);
}

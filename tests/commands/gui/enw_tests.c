...
Test(enw_event, valid_egg, .timeout = 2) {
    ...
    cr_assert(strstr(response, "enw #42 #7 3 4") != NULL, "Should output correct enw event");
    close(read_fd);
    free(response);
...
Test(enw_event, null_egg, .timeout = 2) {
    enw_event(NULL);
}

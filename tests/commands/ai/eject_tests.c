/*
** EPITECH PROJECT, 2025
** Zappy
** File description:
** Eject Command Tests
*/

#include "command_handler/ai_commands.h"
#include "command_handler/command.h"
#include "connection/client.h"
#include "connection/server.h"
#include "connection/writing_buffer.h"
#include "constants.h"
#include "game/game.h"
#include "game/game_constants.h"
#include "game/incantation.h"
#include "map/coordinates.h"
#include "map/map.h"
#include "map/resources.h"
#include "map/tile.h"
#include "options_parser/options.h"
#include "team/player/player.h"
#include "vector.h"
#include <criterion/criterion.h>
#include <stdlib.h>
#include <string.h>

static server_t *create_test_server(void)
{
    server_t *server = malloc(sizeof(server_t));
    if (!server)
        return NULL;

    server->options = malloc(sizeof(server_options_t));
    if (!server->options) {
        free(server);
        return NULL;
    }

    server->options->debug = false;
    server->options->port = 4242;
    server->options->width = 10;
    server->options->height = 10;
    server->options->frequency = 100;
    server->game = NULL;

    // Initialize clients array to NULL
    for (int i = 0; i < MAX_CLIENTS; i++) {
        server->clients[i] = NULL;
    }
    return server;
}

static void destroy_test_server(server_t *server)
{
    if (!server)
        return;
    if (server->options)
        free(server->options);
    free(server);
}

static client_t *create_test_client(server_t *server)
{
    client_t *client = malloc(sizeof(client_t));
    if (!client)
        return NULL;

    client->sockfd = -1;
    client->server = server;
    client->is_gui = false;
    client->player = NULL;
    client->index = 0;
    client->command_buffer = vector_new(sizeof(command_t *));
    client->writing_buffer = vector_new(sizeof(char *));
    return client;
}

static void destroy_test_client(client_t *client)
{
    if (!client)
        return;
    if (client->writing_buffer) {
        clear_writing_buffer(client);
        vector_destroy(client->writing_buffer);
        client->writing_buffer = NULL;
    }
    if (client->command_buffer) {
        vector_destroy(client->command_buffer);
        client->command_buffer = NULL;
    }
    free(client);
}

static team_t *create_test_team(const char *name, game_t *game)
{
    team_t *team = malloc(sizeof(team_t));
    if (!team)
        return NULL;

    team->name = strdup(name);
    team->players = vector_new(sizeof(player_t *));
    team->eggs = vector_new(sizeof(egg_t *));
    team->game = game;
    return team;
}

static void destroy_test_team(team_t *team)
{
    if (!team)
        return;
    if (team->name)
        free(team->name);
    if (team->players)
        vector_destroy(team->players);
    if (team->eggs)
        vector_destroy(team->eggs);
    free(team);
}

static game_t *create_test_game(void)
{
    server_t *server = create_test_server();
    if (!server)
        return NULL;

    game_t *game = malloc(sizeof(game_t));
    if (!game) {
        destroy_test_server(server);
        return NULL;
    }

    game->server = server;
    game->map = create_map(10, 10, server);
    game->incantations = vector_new(sizeof(incantation_t *));
    game->next_player_id = 1;
    game->next_egg_id = 1;
    game->tick_rate = 100;
    game->game_tick = 0;

    // Create teams array (NULL-terminated)
    game->teams = malloc(sizeof(team_t *) * 3);
    game->teams[0] = create_test_team("team1", game);
    game->teams[1] = create_test_team("team2", game);
    game->teams[2] = NULL;

    server->game = game;
    return game;
}

static void destroy_test_game(game_t *game)
{
    if (!game)
        return;

    // Clean up incantations vector carefully - but don't destroy the
    // incantations themselves as they might be destroyed by update_incantation
    if (game->incantations) {
        vector_destroy(game->incantations);
    }

    // Destroy map after cleaning up incantations
    if (game->map)
        destroy_map(game->map);

    // Clean up teams
    if (game->teams) {
        for (int i = 0; game->teams[i]; i++) {
            destroy_test_team(game->teams[i]);
        }
        free(game->teams);
    }

    // Clean up server last
    if (game->server)
        destroy_test_server(game->server);

    free(game);
}

static player_t *create_test_player(game_t *game, pos_t pos, level_t level)
{
    client_t *client = create_test_client(game->server);
    if (!client)
        return NULL;

    player_t *player = malloc(sizeof(player_t));
    if (!player) {
        destroy_test_client(client);
        return NULL;
    }

    // Initialize player
    player->id = game->next_player_id++;
    player->pos = pos;
    player->orientation = NORTH;
    player->level = level;
    player->tick_cooldown = 0;
    player->hunger_cooldown = 126;
    player->doing_action = false;
    player->in_incantation = false;
    player->team = game->teams[0];
    player->client = client;

    // Initialize inventory to zero
    for (resource_t r = FOOD; r < RESOURCE_COUNT; r++) {
        player->inventory[r] = 0;
    }
    player->inventory[FOOD] = 10;      // Give some food

    client->player = player;

    // Add player to the tile at the specified position
    tile_t *tile = get_tile(game->map, pos);
    if (tile) {
        const vector_vtable_t *vtable = vector_get_vtable(tile->players);
        vtable->push_back(tile->players, &player);
    }

    return player;
}

static void destroy_test_player(player_t *player)
{
    if (!player)
        return;
    if (player->client) {
        destroy_test_client(player->client);
        player->client = NULL;
    }
    free(player);
}

Test(eject_command, eject_basic, .timeout = 2)
{
    server_t *server = create_test_server();
    server->game = create_test_game();
    client_t *client = create_test_client(server);
    client->player = create_test_player(server->game, (pos_t){2, 2}, 1);
    command_t cmd = {0};
    cmd.name = strdup("Eject");
    cmd.argc = 0;
    cmd.argv = NULL;
    eject_command(client, &cmd);
    free(cmd.name);
    destroy_test_player(client->player);
    destroy_test_client(client);
    destroy_test_game(server->game);
    destroy_test_server(server);
}

Test(eject_command, eject_multiple_players, .timeout = 2)
{
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {3, 3};
    client_t *ejector_client = create_test_client(server);
    player_t *ejector = create_test_player(game, pos, 1);
    ejector->orientation = EAST;
    ejector_client->player = ejector;
    player_t *pushed1 = create_test_player(game, pos, 1);
    player_t *pushed2 = create_test_player(game, pos, 1);
    pos_t orig1 = pushed1->pos;
    pos_t orig2 = pushed2->pos;
    command_t cmd = {0};
    cmd.name = strdup("Eject");
    cmd.argc = 0;
    cmd.argv = NULL;
    eject_command(ejector_client, &cmd);
    cr_assert_eq(ejector->pos.x, pos.x);
    cr_assert_eq(ejector->pos.y, pos.y);
    cr_assert_eq(pushed1->pos.x, (orig1.x + 1) % 10);
    cr_assert_eq(pushed1->pos.y, orig1.y);
    cr_assert_eq(pushed2->pos.x, (orig2.x + 1) % 10);
    cr_assert_eq(pushed2->pos.y, orig2.y);
    cr_assert_neq(ejector->tick_cooldown, 0);
    free(cmd.name);
    destroy_test_player(ejector);
    destroy_test_player(pushed1);
    destroy_test_player(pushed2);
    destroy_test_client(ejector_client);
    destroy_test_game(game);
    destroy_test_server(server);
}

Test(eject_command, eject_no_other_players, .timeout = 2)
{
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {4, 4};
    client_t *ejector_client = create_test_client(server);
    player_t *ejector = create_test_player(game, pos, 1);
    ejector->orientation = SOUTH;
    ejector_client->player = ejector;
    command_t cmd = {0};
    cmd.name = strdup("Eject");
    cmd.argc = 0;
    cmd.argv = NULL;
    eject_command(ejector_client, &cmd);
    cr_assert_eq(ejector->pos.x, pos.x);
    cr_assert_eq(ejector->pos.y, pos.y);
    free(cmd.name);
    destroy_test_player(ejector);
    destroy_test_client(ejector_client);
    destroy_test_game(game);
    destroy_test_server(server);
}

Test(eject_command, eject_with_egg_on_tile, .timeout = 2)
{
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {5, 5};
    client_t *ejector_client = create_test_client(server);
    player_t *ejector = create_test_player(game, pos, 1);
    ejector->orientation = NORTH;
    ejector_client->player = ejector;
    // Create an egg on the same tile
    egg_t *egg = malloc(sizeof(egg_t));
    cr_assert_not_null(egg, "Egg allocation should succeed");
    egg->pos = pos;
    egg->team = game->teams[0];
    egg->id = 42;
    tile_t *tile = get_tile(game->map, pos);
    cr_assert_not_null(tile, "Tile should not be NULL");
    const vector_vtable_t *vtable = vector_get_vtable(tile->eggs);
    vtable->push_back(tile->eggs, &egg);
    command_t cmd = {0};
    cmd.name = strdup("Eject");
    cmd.argc = 0;
    cmd.argv = NULL;
    eject_command(ejector_client, &cmd);
    // Egg should remain on the same tile
    egg_t *egg_on_tile = *(egg_t **)vtable->at(tile->eggs, 0);
    cr_assert_eq(egg_on_tile, egg, "Egg should not be moved by eject");
    cr_assert_eq(egg_on_tile->pos.x, pos.x);
    cr_assert_eq(egg_on_tile->pos.y, pos.y);
    free(cmd.name);
    // Clean up
    vtable->erase(tile->eggs, 0);
    free(egg);
    destroy_test_player(ejector);
    destroy_test_client(ejector_client);
    destroy_test_game(game);
    destroy_test_server(server);
}

Test(eject_command, eject_players_and_eggs, .timeout = 2)
{
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {6, 6};
    client_t *ejector_client = create_test_client(server);
    player_t *ejector = create_test_player(game, pos, 1);
    ejector->orientation = WEST;
    ejector_client->player = ejector;
    // Add another player to be ejected
    player_t *pushed = create_test_player(game, pos, 1);
    pos_t orig = pushed->pos;
    // Add an egg to the same tile
    egg_t *egg = malloc(sizeof(egg_t));
    cr_assert_not_null(egg, "Egg allocation should succeed");
    egg->pos = pos;
    egg->team = game->teams[0];
    egg->id = 43;
    tile_t *tile = get_tile(game->map, pos);
    cr_assert_not_null(tile, "Tile should not be NULL");
    const vector_vtable_t *vtable_eggs = vector_get_vtable(tile->eggs);
    vtable_eggs->push_back(tile->eggs, &egg);
    command_t cmd = {0};
    cmd.name = strdup("Eject");
    cmd.argc = 0;
    cmd.argv = NULL;
    eject_command(ejector_client, &cmd);
    // Player should be moved, egg should not
    cr_assert_neq(pushed->pos.x, orig.x, "Pushed player's x should change");
    cr_assert_eq(pushed->pos.y, orig.y, "Pushed player's y should not change");
    egg_t *egg_on_tile = *(egg_t **)vtable_eggs->at(tile->eggs, 0);
    cr_assert_eq(egg_on_tile, egg, "Egg should not be moved by eject");
    cr_assert_eq(egg_on_tile->pos.x, pos.x);
    cr_assert_eq(egg_on_tile->pos.y, pos.y);
    free(cmd.name);
    // Clean up
    vtable_eggs->erase(tile->eggs, 0);
    free(egg);
    destroy_test_player(ejector);
    destroy_test_player(pushed);
    destroy_test_client(ejector_client);
    destroy_test_game(game);
    destroy_test_server(server);
}

...
static server_t *create_test_server(void)
{
    ...
    server->options->display_eggs = true;
    server->game = NULL;
...
Test(look_command, look_null_vision, .timeout = 2) {
    // Simulate look() returning NULL by temporarily replacing the function pointer
    // This requires look_command to call a function pointer or be mockable.
    // If not possible, this test is a placeholder for when dependency injection is available.
    // For now, we just check that the function does not crash if vision is NULL.
    server_t *server = create_test_server();
    server->game = create_test_game();
    ...
    cmd.argc = 0;
    cmd.argv = NULL;
    // Simulate by freeing the map or corrupting player position if needed
    // Here, forcibly set map to NULL
    game->map = NULL;
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
    cr_assert_neq(read_fd, -1, "Pipe creation should succeed");
...
Test(look_command, look_basic, .timeout = 2) {
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {2, 2};

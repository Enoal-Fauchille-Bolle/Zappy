...
Test(broadcast_command, broadcast_basic, .timeout = 2) {
    server_t *server = create_test_server();
    ...
    client->player = player;
    // Create a real command for broadcast
    command_t cmd = {0};
    cmd.name = strdup("Broadcast");
    cmd.argc = 1;
    cmd.argv = malloc(sizeof(char *) * 2);
    cmd.argv[0] = strdup("Hello world!");
    cmd.argv[1] = NULL;
    broadcast_command(client, &cmd);

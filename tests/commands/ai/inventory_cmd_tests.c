...
// --- inventory_command edge case test ---

Test(inventory_command, inventory_null_inventory, .timeout = 2) {
    server_t *server = create_test_server();
    ...
    client->player = player;
    command_t cmd = {0};
    cmd.name = strdup("Inventory");
    cmd.argc = 0;
    cmd.argv = NULL;
    // Simulate check_inventory() returning NULL by setting player->client to NULL or corrupting player
    // (Assumes check_inventory returns NULL if player is NULL)
    player->client = NULL;
    client->player = NULL; // Simulate player being NULL
    int read_fd, write_fd;
    read_fd = create_test_pipe(&write_fd);
    cr_assert_neq(read_fd, -1, "Pipe creation should succeed");
    client->sockfd = write_fd;
    inventory_command(client, &cmd);
    ...
    cr_assert_str_eq(response, "ko\n", "Should return 'ko\\n' if check_inventory() returns NULL");
    close(read_fd);
    free(response);
    free(cmd.name);
...
Test(inventory_command, inventory_basic, .timeout = 2) {
    server_t *server = create_test_server();
    server->game = create_test_game();
    game_t *game = server->game;
    pos_t pos = {2, 2};

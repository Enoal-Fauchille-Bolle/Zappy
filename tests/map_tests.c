...
Test(map, get_tile_outside_size)
{
    ...
    tile_t *tile = get_tile(map, 10, 10);
    // Same as above, accessing tile at 0, 0
    tile_t *alternate_tile = get_tile(map, 0, 0);
...
Test(map, modify_tile)
{
    ...
    tile_t *tile_new_ptr = get_tile(map, 5, 5);
    cr_assert_eq(tile_new_ptr->food, 10, "Food should be set to 10");
    cr_assert_eq(tile_new_ptr->linemate, 5, "Linemate should be set to 5");
    destroy_map(map);
}
...
Test(map, create_map_valid)
{
    ...
    for (size_t i = 0; i < 100; i++) {
        cr_assert_eq(map->tiles[i].food, 0, "Food should be initialized to 0");
        cr_assert_eq(
            map->tiles[i].linemate, 0, "Linemate should be initialized to 0");
    }
    destroy_map(map);
}

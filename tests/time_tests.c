...
Test(time_functions, get_current_time_ms_returns_positive_value, .timeout = 2)
{
    ...
    cr_assert_gt(
        time_ms, 0, "get_current_time_ms should return a positive value");
}
...
Test(time_functions, get_current_time_ms_is_monotonic, .timeout = 2)
{
    ...
    cr_assert_gt(time2, time1,
        "Time should be monotonic (second call should return greater value)");
}
...
Test(time_functions, get_current_time_ms_precision, .timeout = 2)
{
    ...
    cr_assert_leq(
        diff, 10, "Consecutive calls should have minimal time difference");
}
...
Test(
    time_functions, wait_remaining_tick_time_with_fast_tick_rate, .timeout = 3)
{
    server_t *server =
        create_test_server(1000);      // 1000 ticks per second = 1ms per tick
    ...
    // Should wait approximately 1ms (allowing some tolerance for system
    // overhead)
...
Test(
    time_functions, wait_remaining_tick_time_with_slow_tick_rate, .timeout = 3)
{
    server_t *server =
        create_test_server(10);      // 10 ticks per second = 100ms per tick
    cr_assert_not_null(server, "Failed to create test server");
    ...
    cr_assert_leq(
        elapsed, 110, "Should not wait more than 110ms (allowing tolerance)");
...
{
    server_t *server =
        create_test_server(100);      // 100 ticks per second = 10ms per tick
    cr_assert_not_null(server, "Failed to create test server");
    ...
    cr_assert_leq(
        elapsed, 10, "Should not wait more than 10ms (allowing tolerance)");
...
Test(time_functions, wait_remaining_tick_time_with_overtime, .timeout = 2)
{
    ...
    long long tick_start =
        start_time - 15;      // Simulate 15ms already elapsed (overtime)
    ...
    cr_assert_leq(
        elapsed, 2, "Should not wait when tick time already exceeded");
...
Test(time_functions, wait_remaining_tick_time_null_server_safety, .timeout = 2)
{
    ...
    // In practice, this would likely segfault, but the test documents expected
    // behavior Since we can't actually test with NULL (would crash), we'll
    // verify that a valid call works and document the edge case
    cr_assert(1,
        "Test placeholder for null server case - would segfault in practice");
}
...
Test(time_functions, tick_duration_calculation_accuracy, .timeout = 2)
{
    ...
    double expected_durations[] = {
        1000.0, 100.0, 20.0, 10.0, 1.0};      // in milliseconds
    size_t num_tests = sizeof(test_rates) / sizeof(test_rates[0]);

    for (size_t i = 0; i < num_tests; i++) {
        server_t *server = create_test_server(test_rates[i]);
        cr_assert_not_null(
            server, "Failed to create test server for rate %u", test_rates[i]);
        ...
        cr_assert_float_eq(calculated_duration, expected_durations[i], 0.001,
            "Tick duration calculation should be accurate for rate %u",
            test_rates[i]);
...
{
    server_t *server =
        create_test_server(200);      // 200 ticks per second = 5ms per tick
    cr_assert_not_null(server, "Failed to create test server");
    ...
    for (int i = 0; i < num_cycles; i++) {
        ...
        cr_assert_geq(
            cycle_elapsed, 4, "Cycle %d should take at least 4ms", i);
        cr_assert_leq(
            cycle_elapsed, 10, "Cycle %d should not take more than 10ms", i);
    }
...
Test(time_functions, extreme_tick_rates, .timeout = 2)
{
    ...
    server_t *fast_server =
        create_test_server(10000);      // 10000 ticks/sec = 0.1ms per tick
    ...
    cr_assert_leq(
        end - start, 2, "Very high tick rate should have minimal wait time");
    ...
    cr_assert_eq(slow_server->game->tick_rate, 1,
        "Slow server should have tick rate of 1");

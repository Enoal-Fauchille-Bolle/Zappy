...
static void option_debug(server_options_t *options)
{
    ...
    debug_server(true, "Port: %d", options->port);
    debug_server(true, "Width: %zu", options->width);
    debug_server(true, "Height: %zu", options->height);
    for (size_t i = 0; options->teams && options->teams[i]; i++) {
        debug_server(true, "Team %zu: %s", i + 1, options->teams[i]);
    }
    debug_server(true, "Clients: %zu", options->clients_nb);
    debug_server(true, "Frequency: %u", options->frequency);
    debug_server(true, "Help: %d", options->help);
    debug_server(true, "Debug: %d", options->debug);
    debug_server(true, "Error: %d", options->error);
}

...
/**
 * @brief Handles the take command to take a resource from the tile.
 *
 * This function checks the command arguments, validates the resource, and
 * attempts to take the resource from the tile where the player is located.
 * If successful, it updates the player's inventory and sends an acknowledgment
...
void take_command(client_t *client, command_t *command)
{
    ...
    if (check_resource_validity(client, command, resource) == FAILURE)
        return;
    if (!take_resource(client->player, client->server->game->map, resource)) {
        write(client->sockfd, "ko\n", 3);
    ...
    client->player->tick_cooldown = TAKE_COMMAND_COOLDOWN;
